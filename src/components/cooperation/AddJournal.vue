  ...
  methods: {
    ...
    submitAll() {
      ...
      if (that.selectDate == "") {
        ...
        ThisFormData = that.selectDate.replace(/-/g, "/");
      }
      ...
      post("/API/PoorHelpAPI/SaveWorkLog", params)
        .then(resp => {
          Utils.hideLoading(that);
          if (resp.ret != "0") {
            Utils.hideLoading(that);
